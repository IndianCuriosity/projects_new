"""
Capstone:
FX Vol Research Agent = RAG + Memory + Tool + LLM synthesis.
"""
...
vectorstore = Chroma.from_documents(
    ...
    collection_name="fx_vol_capstone",
    persist_directory="./chroma_fx_vol_capstone",
)
...
prompt = ChatPromptTemplate.from_messages([
    ("system", """
You are an FX volatility research agent.

Use:
1. Retrieved context
2. User query
3. Tool results if provided

Give:
- Market interpretation
- Trade idea
- Risks
- What data to check next
...
if __name__ == "__main__":
    rv = realized_vol.invoke({"daily_returns_csv": "0.005,-0.004,0.006,-0.003,0.004"})
    print(research(
        "Should I buy EURUSD gamma before an ECB event?",
        tool_results=f"Recent annualized realized vol estimate: {rv:.2%}"
    ))
